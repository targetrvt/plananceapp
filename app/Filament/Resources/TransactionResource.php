        ...
        return $form
            ->schema([
                ...
                Forms\Components\Section::make(__('transaction.form.transaction_details.section'))
                    ->schema([
                        ...
                        Forms\Components\Select::make('financial_goal_id')
                            ->label(__('transaction.form.transaction_details.financial_goal.label'))
                            ->relationship(
                                'financialGoal',
                                'name',
                                fn ($query) => $query->where('user_id', auth()->id())->orderBy('name')
                            )
                            ->searchable()
                            ->nullable()
                            ->visible(fn (Get $get): bool => $get('category') === 'savings'),
                ...
                Forms\Components\Hidden::make('receipt_path')
                    ->dehydrateStateUsing(fn ($state, $record) => $record && $record->receipt_image ? $record->receipt_image : null
                    ),
            ]);
    }
                ...
                [
                    ...
                    'content' => [
                        [
                            ...
                            5. category: (categorize this transaction as one of: salary, investment, gift, refund, other_income, food, shopping, entertainment, transportation, housing, utilities, health, education, travel, unhealthy_habits, other_expense, savings)
