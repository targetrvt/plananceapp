...
use App\Services\Ai\TrackedChatManager;
use Exception;
...
class CustomChatgptAgent extends BaseChatgptAgent
{
    ...
    {
        $userModel = auth()->user();
        if (! $userModel?->hasAiAccess()) {
            $blocked = __('messages.ai_access.chat_blocked', ['email' => (string) config('planance.contact_ai_email')]);
            $this->messages[] = ['role' => 'assistant', 'content' => $blocked];

            request()->session()->put($this->getSessionKey(), $this->messages);
        ...
        try {
            ...
            TrackedChatManager::makeTracked(
                $chat,
                auth()->id(),
                AiUsageLog::FEATURE_PLANANCE_SUPPORT,
            )->send();
